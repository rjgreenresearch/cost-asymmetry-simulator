...
def main(argv=None) -> int:
    ...
    for r in marg_rows:
        ...
        net = max(0, daily - annual / 365)
        if net > 0 and unit_cost > 0:
            r["days_per_1000M"] = round(1e9 / unit_cost / net, 1)
        else:
            r["days_per_1000M"] = "∞"
    ...
    # ── Write outputs ─────────────────────────────────────────────────────────
    if "json" in report_formats or True:   # always write JSON
